    ...
    def _generate_day_image(self, route_df, day):
        ...
        for index, row in day_df.iterrows():
            ...
            if row['city'] in city_features:
                ...
                x_images[city_index, row_value, col_value] += self.args.weight

            if row['type'] in type_features:
                type_index = type_features.index(row['type'])
                type_index += len(city_features)
            ...
            if row['infection_case'] in reason_features:
                ...
                reason_index += len(city_features) + len(type_features)
